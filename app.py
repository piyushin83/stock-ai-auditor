...
# ═══════════════════════════════════════════════════════════════════════
# CHANGE 1: 7-Day news-weighted projection (replaces 30-day flat target)
# ═══════════════════════════════════════════════════════════════════════
def compute_7day_target(df, forecast, all_news, is_death_cross, fx):
    """
    Returns a 7-trading-day price target (already FX-adjusted) that blends:
      1. Prophet yhat 7 business days ahead (structural trend baseline)
      2. Recency-weighted news sentiment from the past 7 days
         — most recent headline = highest weight (1/age_in_days)
         — ±1 polarity unit → ±3% price nudge, capped total at ±5%
      3. Last-5-day price momentum (half-weight carry forward)
      4. Death-cross penalty: −2% extra if 50MA < 200MA
    """
    # 1. Prophet base — 7 business days ahead
    try:
    ...
    # 2. Recency-weighted sentiment (last 7 days only)
    ...
    for item in all_news:
        ...
        if age > 7:
            continue
        w = 1.0 / age          # today ≈ weight 1440, yesterday ≈ 0.5, 7d ago ≈ 0.14
        s_sum += item['sentiment'] * w
        w_sum += w
    avg_sentiment = s_sum / w_sum if w_sum > 0 else 0.0
    news_adj = avg_sentiment * 0.03   # ±3% per unit of polarity

    # 3. Short-term momentum (last 5 days, half-weight)
    momentum_adj = float(df['y'].pct_change().tail(5).mean()) * 0.5

    # 4. Death-cross penalty
...
def format_candidates_html(candidates):
    """Render scored picks as inline badges showing analyst rating + upside %."""
    if not candidates:
...
def generate_investment_roadmap(df_results, sector_data, total_value, display_currency):
    """
    Personalised 4-phase investment roadmap.
    Every ticker recommendation is scored live from yfinance using:
      analyst consensus, price target upside, EPS/revenue growth, 52-week momentum.
    Generic static lists are gone — only the highest-scoring non-owned tickers appear.
    """
    sym = "$" if display_currency == "USD" else "€"

    # 10 candidates per sector → live-scored → top 3 shown
    SECTOR_BENCHMARKS = {
    ...
    roadmap.append({'phase': 'PHASE 4 — LONG-TERM OPTIMISATION (6–12 Months)', 'icon': '🎯',
                    'color': '#2e7d32', 'actions': phase4_actions})
...
def render_investment_roadmap(roadmap, overweight, underweight, missing, sym):
    ...
    st.caption("Phased action plan vs S&P 500 benchmark weights. "
               "Ticker picks scored live: analyst consensus, price target upside, EPS/revenue growth & 52-week momentum.")
...
if run_audit:
    with st.spinner(f"Analyzing {user_query}... (may take a moment)"):
        ...
        if df is not None:
            ...
            is_death_cross = (df['50_Day_Moving_Average'].iloc[-1] <
                              df['200_Day_Moving_Average'].iloc[-1])
            ...
            # ── CHANGE 1 applied: fetch news then compute news-adjusted 7-day target ──
            all_news = get_enhanced_news(ticker)

            if forecast is not None:
                target_p_7 = compute_7day_target(df, forecast, all_news, is_death_cross, fx)
            else:
                # Fallback when Prophet failed: simple 5-day momentum carry
            ...
            if ai_roi_7 > 0.5:           score += 30   # ← uses 7-day ROI
            ...
            # ── CHANGE 1: metrics now display 7-day labels ───────────────────────────
            ...
            st.markdown(f'<div class="verdict-box {v_col}">Strategic Verdict: {verdict} | {action}</div>',
                        unsafe_allow_html=True)
            st.markdown(f'<div class="stop-loss-box">🛑 AGGRESSIVE STOP LOSS: {sym}{sl_price:,.2f}</div>',
                        unsafe_allow_html=True)
            ...
            with col_l:
                ...
                st.table(pd.DataFrame({
                    ...
                    "Status":  [f"{health['ROE']*100:.1f}%", f"{health['PB']}x",
                                health['Debt'], health['CurrentRatio']],
                    "Rating":  ["✅ Prime"    if health['ROE'] > 0.15 else "⚠️ Weak",
                                "✅ Healthy"  if health['PB']  < 3.0  else "⚠️ Expensive",
                                "✅ Safe", "✅ Liquid"]
                }))
    ...
    if result and result['type'] == 'tabular':
        ...
        if results_list:
            ...
            if 'Sector' in df_results.columns:
                ...
                # Original quick tips (kept intact)
