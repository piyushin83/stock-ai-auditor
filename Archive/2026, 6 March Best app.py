...
# -------------------------------
# 0. CHECK DEPENDENCIES (with helpful messages)
# -------------------------------
missing_libs = []
try:
...
# FIX 1: resolve_smart_ticker now robustly fetches company name from multiple sources
@st.cache_data(ttl=3600)
def resolve_smart_ticker(user_input):
    ...
    try:
        ...
        # Try fast_info first, then fall back to full info
...
# FIX 2: Fetch sector from yfinance (live) with Finviz fallback
@st.cache_data(ttl=3600)
def get_ticker_sector_online(ticker):
    """Fetch sector for a ticker from yfinance, with Finviz scrape as fallback."""
    try:
        rate_limiter.wait()
...
# 6. PORTFOLIO ANALYSIS & DIVERSIFICATION
def analyze_ticker_basic(ticker, display_currency):
    try:
        ...
        # FIX 2: Always fetch sector live from yfinance
        sector = info.get('sector', '')
        industry = info.get('industry', '')
...
# FIX 3: Full investment roadmap engine
def generate_investment_roadmap(df_results, sector_data, total_value, display_currency):
    """
    Generate a rich, personalised investment roadmap based on:
    - Current portfolio composition & gaps
    - Sector concentration / underweights
    - Risk profile of existing holdings
    - Actionable phased recommendations with specific tickers
    """
    sym = "$" if display_currency == "USD" else "€"

    # Sector benchmark targets (approximate S&P 500 weightings)
    SECTOR_BENCHMARKS = {
        'Technology': {'target': 28, 'etf': 'XLK', 'growth_tickers': ['MSFT', 'GOOGL', 'META', 'ORCL'], 'div_tickers': ['AAPL', 'CSCO'], 'risk': 'High'},
        'Healthcare': {'target': 13, 'etf': 'XLV', 'growth_tickers': ['UNH', 'LLY', 'ABBV', 'ISRG'], 'div_tickers': ['JNJ', 'PFE', 'MRK'], 'risk': 'Low'},
        'Financial Services': {'target': 13, 'etf': 'XLF', 'growth_tickers': ['JPM', 'V', 'MA', 'BRK-B'], 'div_tickers': ['BAC', 'WFC', 'C'], 'risk': 'Medium'},
        'Consumer Cyclical': {'target': 10, 'etf': 'XLY', 'growth_tickers': ['AMZN', 'TSLA', 'NKE', 'MCD'], 'div_tickers': ['HD', 'LOW'], 'risk': 'High'},
        'Industrials': {'target': 8, 'etf': 'XLI', 'growth_tickers': ['CAT', 'DE', 'RTX', 'GE'], 'div_tickers': ['HON', 'MMM'], 'risk': 'Medium'},
        'Communication Services': {'target': 8, 'etf': 'XLC', 'growth_tickers': ['GOOGL', 'META', 'NFLX', 'SPOT'], 'div_tickers': ['VZ', 'T'], 'risk': 'Medium'},
        'Consumer Defensive': {'target': 6, 'etf': 'XLP', 'growth_tickers': ['COST', 'WMT', 'MDLZ'], 'div_tickers': ['PG', 'KO', 'PEP'], 'risk': 'Low'},
        'Energy': {'target': 4, 'etf': 'XLE', 'growth_tickers': ['XOM', 'CVX', 'SLB', 'EOG'], 'div_tickers': ['COP', 'PSX'], 'risk': 'High'},
        'Utilities': {'target': 2, 'etf': 'XLU', 'growth_tickers': ['NEE', 'AES'], 'div_tickers': ['DUK', 'SO', 'D'], 'risk': 'Low'},
        'Real Estate': {'target': 2, 'etf': 'XLRE', 'growth_tickers': ['PLD', 'AMT', 'EQIX'], 'div_tickers': ['O', 'SPG', 'VICI'], 'risk': 'Low'},
        'Basic Materials': {'target': 2, 'etf': 'XLB', 'growth_tickers': ['LIN', 'APD', 'FCX'], 'div_tickers': ['NEM', 'BHP'], 'risk': 'Medium'},
    }

    # Existing tickers in portfolio (to avoid recommending what they already own)
    ...
    # Identify overweight, balanced, and underweight/missing sectors
    ...
    for sector, info in SECTOR_BENCHMARKS.items():
        ...
        gap = current - target
        if gap > 5:
            overweight.append((sector, current, target, gap))
        elif current == 0:
            missing.append((sector, target, info))
        elif gap < -3:
            underweight.append((sector, current, target, abs(gap), info))
    ...
    defensive_weight = sum(current_sectors.get(s, 0) for s in ['Consumer Defensive', 'Utilities', 'Healthcare'])
    num_holdings = len(df_results)
    concentration = df_results.nlargest(3, 'Current Value')['Current Value'].sum() / total_value * 100 if total_value > 0 else 0
    ...
    if overweight:
        for sector, cur, tgt, gap in overweight[:2]:
            phase1_actions.append(f"<b>Trim {sector}</b>: currently {cur:.1f}% vs {tgt}% benchmark — consider reducing by ~{gap:.0f}% (take profits on laggards)")
    if concentration > 50:
        top3 = df_results.nlargest(3, 'Current Value')['Ticker'].tolist()
        phase1_actions.append(f"<b>Reduce concentration</b>: top 3 holdings ({', '.join(top3)}) = {concentration:.0f}% of portfolio. Target under 40%.")
    if not phase1_actions:
        phase1_actions.append("Portfolio is reasonably balanced — no urgent rebalancing needed.")
    roadmap.append({
        'phase': 'PHASE 1 — IMMEDIATE (Now → 30 Days)',
        'icon': '⚡',
        'color': '#c62828',
        'actions': phase1_actions
    })

    # ── PHASE 2: Core gap-filling ────────────────────────────
    phase2_actions = []
    priority_missing = missing[:3]
    for sector, target_pct, info in priority_missing:
        suggested_alloc = total_value * (target_pct / 100)
        candidates = [t for t in info['growth_tickers'] if t not in owned_tickers][:2]
        etf_note = f"or use ETF <span class='roadmap-ticker'>{info['etf']}</span> for instant exposure"
        tickers_html = " ".join([f"<span class='roadmap-ticker'>{t}</span>" for t in candidates])
        risk_badge = f"<span class='risk-badge-{'low' if info['risk']=='Low' else 'med' if info['risk']=='Medium' else 'high'}'>{info['risk']} Risk</span>"
        phase2_actions.append(
            f"<b>Add {sector}</b> {risk_badge}: 0% → target {target_pct}% "
            f"(≈{sym}{suggested_alloc:,.0f}). Consider: {tickers_html} {etf_note}"
        )
    ...
    for sector, cur, tgt, gap, info in underweight[:2]:
        ...
        phase2_actions.append(
            ...
            f"(add ≈{sym}{suggested_alloc:,.0f}). Candidates: {tickers_html}"
        )

    if not phase2_actions:
        phase2_actions.append("Good sector coverage — focus on quality within existing positions.")
    roadmap.append({
        'phase': 'PHASE 2 — CORE BUILD (1–3 Months)',
        'icon': '🏗️',
        'color': '#e65100',
        'actions': phase2_actions
    })

    # ── PHASE 3: Growth & income layer ──────────────────────
    phase3_actions = []

    # Growth layer: check if tech < 20%
    if tech_weight < 15:
        growth_candidates = [t for t in ['NVDA', 'MSFT', 'GOOGL', 'AMD', 'AVGO'] if t not in owned_tickers][:3]
        tc = " ".join([f"<span class='roadmap-ticker'>{t}</span>" for t in growth_candidates])
        phase3_actions.append(f"<b>Growth Layer — AI/Tech</b>: Only {tech_weight:.1f}% tech exposure. Add high-growth names: {tc}")

    # Dividend / income layer
    income_sectors = ['Consumer Defensive', 'Utilities', 'Real Estate']
    income_weight = sum(current_sectors.get(s, 0) for s in income_sectors)
    if income_weight < 8:
        div_picks = [t for t in ['O', 'JNJ', 'PG', 'KO', 'VZ', 'NEE', 'VICI'] if t not in owned_tickers][:4]
        tp = " ".join([f"<span class='roadmap-ticker'>{t}</span>" for t in div_picks])
        phase3_actions.append(f"<b>Income / Dividend Layer</b>: Only {income_weight:.1f}% in defensive/income sectors. "
                               f"Add dividend payers for stability: {tp}")
    ...
    # Bonds / alternatives if equity-heavy
    phase3_actions.append(
    ...
    roadmap.append({
        'phase': 'PHASE 3 — GROWTH & INCOME LAYER (3–6 Months)',
        'icon': '📈',
        'color': '#1565c0',
    ...
    if remaining_missing:
        for sector, target_pct, info in remaining_missing[:3]:
            candidates = [t for t in (info['div_tickers'] + info['growth_tickers']) if t not in owned_tickers][:2]
            tickers_html = " ".join([f"<span class='roadmap-ticker'>{t}</span>" for t in candidates])
            phase4_actions.append(f"<b>Complete {sector} coverage</b> (target {target_pct}%): {tickers_html}")

    phase4_actions.append(
        "<b>Rebalance Annually</b>: Set calendar reminders to review sector weights vs "
        "benchmarks each January. Trim winners above +5% overweight, add to laggards."
    )
    phase4_actions.append(
        "<b>Tax-Loss Harvesting</b>: Before year-end, review positions with unrealised losses "
        "and consider harvesting to offset capital gains."
    )
    phase4_actions.append(
        "<b>Dollar-Cost Averaging</b>: For new positions, deploy capital in 3 tranches "
        "over 6–8 weeks to reduce timing risk."
    )

    roadmap.append({
        'phase': 'PHASE 4 — LONG-TERM OPTIMISATION (6–12 Months)',
        'icon': '🎯',
        'color': '#2e7d32',
...
def render_investment_roadmap(roadmap, overweight, underweight, missing, sym):
    ...
    st.caption("Phased action plan based on your current portfolio composition vs S&P 500 benchmark weights")
...
<div class="roadmap-card">
  ...
  {''.join([f'<div class="roadmap-phase"><p style="margin:0; line-height:1.7; font-size:14px;">{action}</p></div>' for action in phase_data['actions']])}
</div>
""", unsafe_allow_html=True)
...
def suggest_diversification(current_sectors, total_value):
    """Original simple suggestions — kept for backward compat, now supplemented by roadmap."""
    suggestions = []
    sector_examples = {
...
if st.sidebar.button("🚀 Run Deep Audit"):
    with st.spinner(f"Analyzing {user_query}... (may take a moment)"):
        ...
        # FIX 1: Use resolved company name; only fall back to ticker if truly unavailable
        display_name = name if (name and name.strip() and name.strip() != ticker) else None
        if not display_name:
            # Last-resort: try fetching info again directly
            try:
                info = yf.Ticker(ticker).info
        ...
        if df is not None:
            ...
            # DISPLAY — FIX 1: Show real company name in heading
...
if uploaded_file is not None:
    ...
    if result and result['type'] == 'tabular':
        ...
        for idx, row in df_holdings.iterrows():
            ...
            if analysis:
                ...
                results_list.append({
                    'Ticker': ticker,
                    'Name': analysis['name'],
                    'Sector': analysis['sector'],        # FIX 2: live sector lookup
        ...
        if results_list:
            ...
            # Show industry column if present
